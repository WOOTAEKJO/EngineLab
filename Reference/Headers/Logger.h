...
public:
	...
	void Write(LEVEL lev, CATEGORY cat, string msg, Fileds filed = Fileds{});
...
private:
	string FormatTime(chrono::system_clock::time_point time);
	void		Format();
	void		Broadcast();
	uint32_t	ThreadID();
...
private:
	struct RateState
	{
		chrono::steady_clock::time_point last = chrono::steady_clock::now() - chrono::seconds(10);
		size_t suppressed = 0;
	};
